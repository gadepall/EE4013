...
Connect the STLINK to a USB port of the Raspberry Pi.  The hardware connections between the STLINK and STM32 are available in Table \ref{table:pins}. See Fig. \ref{fig:stm32} as well.
\begin{table}[!h]
\centering
\input{./figs/pins.tex}
\caption{}
...
\begin{figure}[!h]
\centering
\includegraphics[width=\columnwidth]{./figs/stm.eps}
...
\end{lstlisting}
\item Make sure that the two pin caps beside the reset button are non-aligned.
...
\item Modify the blinking part of the code in main.c in the STM32F103C8T6 directory as
\begin{lstlisting}
 while (1)
  {
