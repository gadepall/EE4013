...
\section{Connecting the Display}
...
\begin{problem}
Explain the process of generating the number 0 in the above code.
\end{problem}
\solution The relevant line in \textbf{sevenseg.c} is
\begin{lstlisting}
GPIOB->ODR = 0x03FF;//0
\end{lstlisting}
The hexadecimal number on the RHS represents the pin configuration for the pins of port B of STM32F103C8T6, which are numbered PB15-PB0 in that order.  For generating the number 0, according to Table \ref{table:stm_ssd},  Pins PB15-PB8 result in 0x03, since pin PB8 is kept high.  Also, pins PB7-PB0 are also kept high, resulting in FF.

\begin{problem}
	Repeat the above exercise to generate the numbers 1-9 on the display.
\end{problem}	
\begin{problem}
Introduce a delay to generate a decade counter.
\end{problem}
